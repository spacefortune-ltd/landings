...
const preloadSounds = async () => {
    ...
    bufferCache.clickS1 = await loadAudio('./sounds/click1.mp3');
    bufferCache.clickS2 = await loadAudio('./sounds/click2.mp3');
    bufferCache.clickS3 = await loadAudio('./sounds/click.mp3');
};
...
jQuery(function() {
    ...
    jQuery('button').on('click', function() {
        if (click === 1) {
            wheelSource = playSound(bufferCache.clickS1, 0.4);
            if (!musicSource) {
                musicSource = playSound(bufferCache.music, 0.2, true);
            }
            ++click;
            ...
            if (jQuery('.tab.sound').hasClass('active')) {
                wheelSource = playSound(bufferCache.clickS2, 0.4);
            }
        }
